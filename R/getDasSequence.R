...
getDasSequence <- function(source, ranges,
                           class=c("character", "DNAStringSet", "RNAStringSet",
                                   "AAStringSet"))
{
    ...
    message("Quering url: ",
            paste(getDasServer(), "/", source, "/sequence", sep=""))
    ...
    doc <- tryCatch(
        ...
        XMLError=function(e) {
          message("Error: ", e$message)
            if (e$code == 1549 & isLong) {
                message("Maybe URL is too long. Try querying less ranges")
            }
        },
        error=function(e) {
            message("Error: ",
    ...
    for (iseg in seq_along(names(top))) {  # Each Segment
        seg <- top[[iseg]]
        res[[iseg]] <- unlist(xmlApply(seg, .getSequence), use.names=FALSE)
    }
