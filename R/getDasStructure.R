...
{
    url <- paste(getDasServer(), "/", source, "/structure?", "query=", query,
                 sep="")
    message("Quering url: ",
            paste(getDasServer(), "/", source, "/structure?query=", query,
                  sep=""))
    ...
    doc <- tryCatch(
        xmlTreeParse(tf, useInternalNodes = TRUE,error=NULL),
        XMLError=function(e) {
    ...
    for (igroup in seq_along(names(top))) {  # Each Group
        ...
        all_col[[igroup]] <- data.frame(cbind(res_atom [[igroup]],
                                              res_group[[igroup]]))
    }
